...
// ---- tunnel-audio (TNGC-36 follow-up) — embed-blocked tracks for tricorder
// viewscreens, cached in R2 under tunnel-audio/ (a bucket lifecycle rule
// expires them). Gated to TUNNEL_AUDIO_TENANTS slugs — a house-operator
// perk, not a customer feature (storage would balloon at scale). Both
// endpoints sit BEFORE the session gate: GET auths via ?token= (media
// elements can't set headers), PUT via the tenant service token.

async function tunnelAudioAllowed(env: Env, tenantId: string): Promise<boolean> {
  const allowed = (env.TUNNEL_AUDIO_TENANTS ?? "")
    .split(",")
  ...
  return !!row?.slug && allowed.includes(row.slug.toLowerCase());
}

const VIDEO_ID_RE = /^[A-Za-z0-9_-]{11}$/;

app.get("/api/tunnel-audio/:videoId", async (c) => {
  const videoId = c.req.param("videoId");
  if (!VIDEO_ID_RE.test(videoId)) return c.json({ error: "bad video id" }, 400);
  const token = bearerToken(c.req.raw) ?? c.req.query("token") ?? "";
  if (!token) return c.json({ error: "unauthorized" }, 401);
  const authed = new Request(c.req.raw.url, { headers: { authorization: `Bearer ${token}` } });
  const session = await lookupSession(c.env, authed);
  if (!session) return c.json({ error: "unauthorized" }, 401);
  if (!(await tunnelAudioAllowed(c.env, session.tenantId))) {
    return c.json({ error: "tunnel audio is not enabled for this household" }, 403);
  }
  const key = `tunnel-audio/${videoId}`;
  // Single-range support so the media element can seek.
  const rangeHeader = c.req.header("range");
  const m = rangeHeader ? /^bytes=(\d+)-(\d*)$/.exec(rangeHeader) : null;
  const head = await c.env.LIBRARY.head(key);
  if (!head) {
    // not cached yet — ask the house to fetch it (DO dedupes triggers)
    const res = await hub(c, session.tenantId).fetch(
      new Request("https://hub/audio-fetch", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ videoId }),
      }),
    );
    if (res.status === 409) return c.json({ error: "Computer offline" }, 409);
    return c.json({ ok: true, preparing: true }, 202);
  }
  ...
  if (m) {
    ...
    const obj = await c.env.LIBRARY.get(key, { range: { offset: start, length: end - start + 1 } });
...
app.put("/api/tunnel-audio/:videoId", async (c) => {
  const videoId = c.req.param("videoId");
  if (!VIDEO_ID_RE.test(videoId)) return c.json({ error: "bad video id" }, 400);
  ...
  const tenant = await c.env.DB.prepare("SELECT id FROM tenants WHERE service_token_hash = ?")
    ...
    .first<{ id: string }>();
  if (!tenant) return c.json({ error: "unauthorized" }, 401);
  if (!(await tunnelAudioAllowed(c.env, tenant.id))) {
    return c.json({ error: "tunnel audio is not enabled for this household" }, 403);
  }
  if (!c.req.raw.body) return c.json({ error: "empty body" }, 400);
  const contentType = c.req.header("content-type") ?? "video/mp4";
  await c.env.LIBRARY.put(`tunnel-audio/${videoId}`, c.req.raw.body, {
    httpMetadata: { contentType },
  });
  return c.json({ ok: true, videoId }, 201);
});
