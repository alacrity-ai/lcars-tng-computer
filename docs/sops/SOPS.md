...
| Situation | SOP | TL;DR |
|-----------|-----|-------|
| **Add a new panel to the wall** | [adding-new-panels.md](adding-new-panels.md) | Props in shared → component → registry (total Record = compile-time safety). **Three surfaces**: the wall reads source, the tricorder viewscreen reads a PREBUILT bundle (`build:vs` + deploy, or phones show "not yet installed"), and a skill is what makes the model reach it. |
| **Add a plugin (sidecars + MCP tool + skill + wall panel + tricorder face)** | [adding-plugins.md](adding-plugins.md) | **Pick your kind first**: sidecar (`lighting`), host service (`claudeops`), or cloud-native (`calendar` — no manifest, alive with the Computer off). Manifest-driven, boot-time merged, zero trace when disabled; pinpoint fence holes, composite panels only, zero-dep MCP. The manifest's required `ui` block gives the phone tile its color + icon; the tricorder face itself is hand-wired in core (step 7). **One tile per plugin, not per feature** — a family of things gets a submenu (`games`). |
