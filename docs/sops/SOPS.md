...
| **Run / rebuild / debug the Computer's Docker fence** | [computer-container.md](computer-container.md) | `make computer` = containerized session (default-deny egress, host secrets absent, repo mounted rw). Allowlist edits need no rebuild. `make computer-bare` = unfenced fallback. |
